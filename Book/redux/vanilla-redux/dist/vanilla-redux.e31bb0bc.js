...
const store = (0, _redux.createStore)(reducer); // store 라이브러리 대신 DOM

const divToggle = document.querySelector(".toggle");
const counter = document.querySelector("h1");
const btnIncrease = document.querySelector("#increase");
const btnDecrease = document.querySelector("#decrease"); // 액션 = 프로젝트의 상태에 변화를 일으키는 것
// 액션의 이름은 주로 대문자, 문자열 형태로 고유한 값 작성

const TOGGLE_SWITCH = "TOGGLE_SWITCH";
const INCREASE = "INCREASE";
const DECREASE = "DECREASE"; // 액션 객체 생성 함수
...
function reducer() {
  ...
  switch (action.type // type에 따라 작업을 처리함 ( type = 함수 이름 )
  ) {
    case TOGGLE_SWITCH:
      // type이 TOGGLE_SWITCH 일 때
      return { ...state,
        // state에
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "11181" + '/');
